        ...
        <div className="flex flex-wrap gap-2" id="obSubjectGrid">
          {Object.keys(SUBJECTS_BY_ID).map((id) => {
            ...
            return (
              <button
                key={id}
                className={`shrink-0 cursor-pointer whitespace-nowrap rounded-[16px] border-[1.5px] border-ash-line bg-surface px-[13px] py-[7px] text-[12.5px] font-bold text-ash ${selectedSubjects.includes(id) ? "border-thread bg-thread-soft text-thread" : ""}`}
